...
This script favors high-recall discovery but keeps the public scope narrow:
acoustic imaging, imaging sonar, synthetic-aperture acoustic/sonar imaging,
acoustic NLOS, acoustic holography/coded acoustics, differentiable acoustic
rendering/reconstruction, and camera/optical-sonar fusion.
"""
...
TOP_VENUE_PATTERNS = [
    r"\bnature\b", r"nature communications", r"nature photonics", r"nature electronics",
    r"communications physics", r"scientific data", r"light: science",
    r"\bscience\b", r"science advances", r"science robotics",
    r"acm transactions on graphics", r"\btog\b", r"\bsiggraph\b", r"siggraph asia",
    r"pattern analysis and machine intelligence", r"\btpami\b",
    r"transactions on image processing", r"\btip\b",
    r"\bcvpr\b", r"computer vision and pattern recognition", r"\biccv\b", r"\beccv\b", r"\biclr\b", r"\bneurips\b", r"neural information processing systems",
]

QUERIES = [
    "imaging sonar differentiable rendering",
    "neural implicit surface reconstruction imaging sonar",
    "neural volume rendering imaging sonar",
    "forward-looking sonar 3D reconstruction differentiable",
    "synthetic aperture sonar neural reconstruction",
    "coherent synthetic aperture sonar reconstruction",
    "camera sonar fusion 3D reconstruction",
    "acoustic optical sensor fusion sonar",
    "opti acoustic sensor fusion volumetric mapping",
    "sonar visual dataset cross modal underwater perception",
    "acoustic non-line-of-sight imaging",
    "ultrasound synthetic aperture non-line-of-sight imaging",
    "acoustic holography phased array computation",
    "coded acoustic imaging computational acoustics",
    "computational ultrasound complex fields imaging",
    "acousto-optic imaging speckle decorrelation",
    "photoacoustic imaging acoustic hologram",
]

CORE_TERMS = ["acoustic", "sonar", "ultrasound", "photoacoustic", "optoacoustic", "acousto-optic", "acousto optic"]
...
PRIMARY_TERMS = ["imaging sonar", "forward-looking sonar", "synthetic aperture sonar", "camera sonar", "camera-sonar", "acoustic-optical", "acoustic optical", "opti-acoustic", "acoustic non-line-of-sight", "acoustic nlos", "ultrasound synthetic aperture", "acoustic holography", "coded acoustic", "computational ultrasound"]
NEGATIVE_TERMS = ["speech recognition", "music", "audio caption", "radio galaxy", "black hole", "x-ray binary", "tidal disruption", "cell culture", "raman data fusion", "football", "ball games", "millimetre arrays", "co2 leakage", "injection wells", "fuel"]

SESSION = requests.Session()
SESSION.headers.update({
    "User-Agent": "acoustic-optics-imaging-updater/2.1 (https://github.com/ruixv/acoustic-optics-imaging; mailto:%s)" % os.getenv("CROSSREF_MAILTO", "example@example.com")
})
...
def is_top_venue(venue: str) -> bool:
    v = (venue or "").lower()
    return any(re.search(p, v) for p in TOP_VENUE_PATTERNS)
...
def score_candidate(title: str, abstract: str, venue: str, query: str, year: Optional[int]) -> Tuple[int, List[str]]:
    ...
    if is_top_venue(venue):
        score += 4; reasons.append("top-venue")
    if any(t in text for t in PRIMARY_TERMS):
        score += 5; reasons.append("primary acoustic-imaging/fusion term")
    elif any(t in text for t in CORE_TERMS):
        score += 2; reasons.append("acoustic/photoacoustic term")
    if any(t in text for t in IMAGING_TERMS):
        score += 3; reasons.append("imaging/reconstruction term")
    if any(t in text for t in NEGATIVE_TERMS):
        score -= 8; reasons.append("negative-topic-filter")
    q_terms = [w for w in re.split(r"\W+", query.lower()) if len(w) > 3]
    hits = sum(1 for w in q_terms if w in text)
    if q_terms and hits >= max(2, len(q_terms) // 2):
        score += 2; reasons.append("query-title/abstract match")
...
def arxiv_search(query: str, rows: int, from_date: str) -> List[Dict[str, Any]]:
    ...
    for e in root.findall("a:entry", ns):
        ...
        if pub_date:
            try:
                if dt.date.fromisoformat(pub_date) < cutoff: continue
        ...
        for link in e.findall("a:link", ns):
            if link.attrib.get("title") == "pdf" or link.attrib.get("type") == "application/pdf": pdf_url = link.attrib.get("href", "")
...
def merge_candidates(new_items: Iterable[Dict[str, Any]], existing: List[Dict[str, Any]], verified: List[Dict[str, Any]], min_score: int) -> List[Dict[str, Any]]:
    ...
    for item in new_items:
        ...
        if k in by_key:
            ...
            item["first_seen"] = today; item["last_seen"] = today; item["times_seen"] = 1; by_key[k] = item
...
def main() -> int:
    ...
    args = ap.parse_args()
    today = dt.date.today(); from_date = (today - dt.timedelta(days=args.since_days)).isoformat()
    verified = load_json(PAPERS_PATH, []) + load_json(DATA_DIR / "focus_papers.json", [])
    existing = load_json(CANDIDATES_PATH, [])
    all_new: List[Dict[str, Any]] = []; errors: List[Dict[str, str]] = []
    for q in QUERIES:
        for source_name, func in [("Crossref", crossref_search), ("arXiv", arxiv_search)]:
            try:
                items = func(q, args.rows, from_date); all_new.extend(items); print(f"{source_name}: {q!r}: {len(items)} raw candidates")
            except Exception as e:
                print(f"ERROR {source_name} {q!r}: {e}"); errors.append({"source": source_name, "query": q, "error": str(e)})
            time.sleep(args.sleep)
    merged = merge_candidates(all_new, existing, verified, args.min_score)
    write_json(CANDIDATES_PATH, merged)
    write_json(RUN_LOG_PATH, {"last_run": dt.datetime.now(dt.timezone.utc).isoformat(), "since_date": from_date, "raw_candidates_seen": len(all_new), "saved_candidates_total": len(merged), "errors": errors, "note": "Candidates are ready for automatic curation-agent audit; high-confidence records may be promoted by scripts/agent_audit.py."})
